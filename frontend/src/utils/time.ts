...
export const formatCreatedTime = (createdAt: string): { time: string; dayInfo: string } => {
  const created = dayjs(createdAt).startOf('second')
  ...
  // 上上周及更早：按周数计算
  else if (diffDays < 28) { // 30天内用"周"显示
  ...
  } else if (created.isAfter(startOfThisMonth) && created.isBefore(now)) {
    dayInfo = `本月初 周${weekDay}`
  }
  // 上个月
  else if (created.isAfter(startOfLastMonth) && created.isBefore(startOfThisMonth)) {
